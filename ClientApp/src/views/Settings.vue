          ...
          <form @submit.prevent="changeProfilePicture">
            <div class="relative mb-6">
              ...
              <div class="relative w-32 h-32 rounded-full overflow-hidden border-4 border-blue-500 cursor-pointer" onclick="document.getElementById('profile-picture').click();">
                <img :src="profilePictureUrl ? profilePictureUrl : 'https://i.ibb.co/2FcqSbJ/image.png'" alt="Profile Picture" class="w-full h-full object-cover" v-if="!loading">
                <div class="absolute inset-0 flex items-center justify-center bg-gray-200" v-if="loading">
                  <span class="text-gray-500 text-sm">Uploading...</span>
                </div>
                <div class="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity bg-black bg-opacity-50 rounded-full" v-if="!loading">
